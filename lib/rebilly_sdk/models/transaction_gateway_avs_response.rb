...
module RebillySdk
  ...
  class TransactionGatewayAvsResponse
    # The response code.
    attr_accessor :code
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::TransactionGatewayAvsResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::TransactionGatewayAvsResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'original_code')
        self.original_code = attributes[:'original_code']
      end

      if attributes.key?(:'original_message')
        self.original_message = attributes[:'original_message']
      end
    end
    ...
    def hash
      [code, message, original_code, original_message].hash
    end
