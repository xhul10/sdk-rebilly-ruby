...
module RebillySdk
  # Truevo credentials object.
  class TruevoCredentials
    # Truevo merchant identifier.
    attr_accessor :mid

    # Truevo terminal identifier.
    attr_accessor :tid

    # Truevo authentication token.
    attr_accessor :token

    # Merchant name for SFTP reconciliation.
    attr_accessor :merchant_name

    # SFTP reconciliation username.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::TruevoCredentials` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::TruevoCredentials`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @tid.nil?
        invalid_properties.push('invalid value for "tid", tid cannot be nil.')
      end
    ...
    def hash
      [mid, tid, token, merchant_name, sftp_username, sftp_private_key].hash
    end
