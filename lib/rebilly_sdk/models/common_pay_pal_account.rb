  ...
  class CommonPayPalAccount
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::CommonPayPalAccount` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::CommonPayPalAccount`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      method_validator = EnumAttributeValidator.new('Object', ['paypal'])
      return false unless method_validator.valid?(@method)
      return false if @customer_id.nil?
      return false if @billing_address.nil?
      status_validator = EnumAttributeValidator.new('Object', ['inactive', 'active', 'deactivated'])
    ...
    def method=(method)
      validator = EnumAttributeValidator.new('Object', ['paypal'])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('Object', ['inactive', 'active', 'deactivated'])
    ...
    def hash
      [id, method, customer_id, billing_address, username, status, created_time, updated_time, custom_fields, risk_metadata].hash
    end
