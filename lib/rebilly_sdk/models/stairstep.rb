  ...
  class Stairstep
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::Stairstep` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::Stairstep`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @brackets.nil?
        invalid_properties.push('invalid value for "brackets", brackets cannot be nil.')
      end
    ...
    def valid?
      ...
      formula_validator = EnumAttributeValidator.new('', ['fixed-fee', 'flat-rate', 'stairstep', 'tiered', 'volume'])
      return false unless formula_validator.valid?(@formula)
    ...
    def formula=(formula)
      validator = EnumAttributeValidator.new('', ['fixed-fee', 'flat-rate', 'stairstep', 'tiered', 'volume'])
      unless validator.valid?(formula)
        fail ArgumentError, "invalid value for \"formula\", must be one of #{validator.allowable_values}."
      end
      @formula = formula
    end
