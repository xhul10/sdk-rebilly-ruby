...
module RebillySdk
  class EcorePayCredentials
    # EcorePay Account ID.
    attr_accessor :account_id

    # EcorePay Account Auth.
    attr_accessor :account_auth
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::EcorePayCredentials` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::EcorePayCredentials`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'account_auth')
        self.account_auth = attributes[:'account_auth']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @account_auth.nil?
        invalid_properties.push('invalid value for "account_auth", account_auth cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @account_auth.nil?
      true
    end
