  ...
  class Attachment
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::Attachment` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::Attachment`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'related_type')
        self.related_type = attributes[:'related_type']
      end

      if attributes.key?(:'related_id')
        self.related_id = attributes[:'related_id']
      end
    ...
    def list_invalid_properties
      ...
      if @related_type.nil?
        invalid_properties.push('invalid value for "related_type", related_type cannot be nil.')
      end

      if @related_id.nil?
        invalid_properties.push('invalid value for "related_id", related_id cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @related_type.nil?
      related_type_validator = EnumAttributeValidator.new('Object', ['customer', 'dispute', 'gateway-timeline-comment', 'invoice', 'organization', 'payment', 'plan', 'product', 'subscription', 'transaction', 'customer-timeline-comment', 'transaction-timeline-comment', 'order-timeline-comment'])
      return false unless related_type_validator.valid?(@related_type)
      return false if @related_id.nil?
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] related_type Object to be assigned
    def related_type=(related_type)
      validator = EnumAttributeValidator.new('Object', ['customer', 'dispute', 'gateway-timeline-comment', 'invoice', 'organization', 'payment', 'plan', 'product', 'subscription', 'transaction', 'customer-timeline-comment', 'transaction-timeline-comment', 'order-timeline-comment'])
      unless validator.valid?(related_type)
        fail ArgumentError, "invalid value for \"related_type\", must be one of #{validator.allowable_values}."
      end
    ...
    def hash
      [id, file_id, related_type, related_id, name, description, created_time, updated_time, _links, _embedded].hash
    end
