  ...
  class TransactionRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::TransactionRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::TransactionRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('', ['3ds-authentication', 'sale', 'authorize', 'setup'])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('', ['3ds-authentication', 'sale', 'authorize', 'setup'])
    ...
    def hash
      [website_id, customer_id, currency, amount, invoice_ids, payment_instruction, billing_address, request_id, gateway_account_id, description, notification_url, redirect_url, custom_fields, risk_metadata, is_processed_outside, is_merchant_initiated, processed_time, type].hash
    end
