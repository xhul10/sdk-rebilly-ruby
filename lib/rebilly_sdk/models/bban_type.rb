  ...
  class BBANType
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::BBANType` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::BBANType`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      account_number_type_validator = EnumAttributeValidator.new('Object', ['IBAN', 'BBAN'])
    ...
    def account_number_type=(account_number_type)
      validator = EnumAttributeValidator.new('Object', ['IBAN', 'BBAN'])
    ...
    def hash
      [method, customer_id, account_number_type, account_number, routing_number, account_type, bank_name, bic, billing_address, custom_fields, risk_metadata].hash
    end
