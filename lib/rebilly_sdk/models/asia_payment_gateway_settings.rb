...
module RebillySdk
  class AsiaPaymentGatewaySettings
    # Use 3DS endpoint.
    attr_accessor :use3_ds_endpoint
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::AsiaPaymentGatewaySettings` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::AsiaPaymentGatewaySettings`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'use3_ds_endpoint')
        self.use3_ds_endpoint = attributes[:'use3_ds_endpoint']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          use3_ds_endpoint == o.use3_ds_endpoint
    end
