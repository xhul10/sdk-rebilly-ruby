  ...
  class CommonOneTimeOrder
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::CommonOneTimeOrder` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::CommonOneTimeOrder`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('', ['pending', 'completed', 'canceled'])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('', ['pending', 'completed', 'canceled'])
    ...
    def hash
      [id, order_type, billing_status, website_id, currency, initial_invoice_id, recent_invoice_id, items, delivery_address, billing_address, activation_time, void_time, coupon_ids, po_number, shipping, notes, status].hash
    end
