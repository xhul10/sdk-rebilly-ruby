  ...
  class AuthorizeNetCredentials
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::AuthorizeNetCredentials` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::AuthorizeNetCredentials`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'api_login_id')
        self.api_login_id = attributes[:'api_login_id']
      end
    ...
    def list_invalid_properties
      ...
      if @api_login_id.nil?
        invalid_properties.push('invalid value for "api_login_id", api_login_id cannot be nil.')
      end

      if @transaction_key.nil?
        invalid_properties.push('invalid value for "transaction_key", transaction_key cannot be nil.')
      end
    ...
    def valid?
      return false if @api_login_id.nil?
      return false if @transaction_key.nil?
      true
    end
    ...
    def hash
      [api_login_id, transaction_key].hash
    end
