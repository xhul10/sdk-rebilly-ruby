  ...
  class Clearhaus3dsServer
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::Clearhaus3dsServer` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::Clearhaus3dsServer`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @acquirer_bin.nil?
        invalid_properties.push('invalid value for "acquirer_bin", acquirer_bin cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @acquirer_bin.nil?
    ...
    def hash
      [name, merchant_id, acquirer_bin, api_key].hash
    end
