...
module RebillySdk
  ...
  class KycSettingsIdentityThresholds
    # The weight of rejectBelow.
    attr_accessor :reject_below

    # The weight of acceptAbove.
    attr_accessor :accept_above
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::KycSettingsIdentityThresholds` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::KycSettingsIdentityThresholds`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'reject_below')
        self.reject_below = attributes[:'reject_below']
      end

      if attributes.key?(:'accept_above')
        self.accept_above = attributes[:'accept_above']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          reject_below == o.reject_below &&
          accept_above == o.accept_above
    end
    ...
    def hash
      [reject_below, accept_above].hash
    end
