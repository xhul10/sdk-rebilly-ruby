...
module RebillySdk
  class TagKycdocumentsBody1
    # The list of kyc document IDs.
    attr_accessor :kyc_document_ids

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'kyc_document_ids' => :'kycDocumentIds'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::TagKycdocumentsBody1` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::TagKycdocumentsBody1`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'kyc_document_ids')
        if (value = attributes[:'kyc_document_ids']).is_a?(Array)
          self.kyc_document_ids = value
        end
      end
    end
    ...
    def list_invalid_properties
      ...
      if @kyc_document_ids.nil?
        invalid_properties.push('invalid value for "kyc_document_ids", kyc_document_ids cannot be nil.')
      end
    ...
    def valid?
      return false if @kyc_document_ids.nil?
      true
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          kyc_document_ids == o.kyc_document_ids
    end
