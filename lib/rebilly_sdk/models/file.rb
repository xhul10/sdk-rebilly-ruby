  ...
  class File
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::File` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::File`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      mime_validator = EnumAttributeValidator.new('Object', ['image/png', 'image/jpeg', 'image/gif', 'application/pdf', 'audio/mpeg'])
      return false unless mime_validator.valid?(@mime)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] mime Object to be assigned
    def mime=(mime)
      validator = EnumAttributeValidator.new('Object', ['image/png', 'image/jpeg', 'image/gif', 'application/pdf', 'audio/mpeg'])
      unless validator.valid?(mime)
        fail ArgumentError, "invalid value for \"mime\", must be one of #{validator.allowable_values}."
      end
      @mime = mime
    end
    ...
    def hash
      [id, name, extension, description, tags, mime, size, width, height, sha1, created_time, updated_time, is_public, _links].hash
    end
