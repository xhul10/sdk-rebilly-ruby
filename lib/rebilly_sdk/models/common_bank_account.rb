  ...
  class CommonBankAccount
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::CommonBankAccount` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::CommonBankAccount`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      method_validator = EnumAttributeValidator.new('Object', ['ach'])
      return false unless method_validator.valid?(@method)
      account_number_type_validator = EnumAttributeValidator.new('Object', ['BBAN', 'IBAN'])
      return false unless account_number_type_validator.valid?(@account_number_type)
      account_type_validator = EnumAttributeValidator.new('Object', ['checking', 'savings', 'other'])
      return false unless account_type_validator.valid?(@account_type)
      status_validator = EnumAttributeValidator.new('Object', ['active', 'deactivated'])
    ...
    def method=(method)
      validator = EnumAttributeValidator.new('Object', ['ach'])
    ...
    def account_number_type=(account_number_type)
      validator = EnumAttributeValidator.new('Object', ['BBAN', 'IBAN'])
    ...
    def account_type=(account_type)
      validator = EnumAttributeValidator.new('Object', ['checking', 'savings', 'other'])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('Object', ['active', 'deactivated'])
    ...
    def hash
      [id, method, customer_id, bank_name, routing_number, account_number_type, account_type, bic, billing_address, fingerprint, last4, status, created_time, updated_time, custom_fields, risk_metadata].hash
    end
