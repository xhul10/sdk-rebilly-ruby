  ...
  class Coupon
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::Coupon` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::Coupon`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'expired_time')
        self.expired_time = attributes[:'expired_time']
      end
    ...
    def list_invalid_properties
      ...
      if @issued_time.nil?
        invalid_properties.push('invalid value for "issued_time", issued_time cannot be nil.')
      end
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('Object', ['draft', 'issued', 'expired'])
      return false unless status_validator.valid?(@status)
      return false if @issued_time.nil?
      true
    end
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('Object', ['draft', 'issued', 'expired'])
    ...
    def hash
      [id, discount, restrictions, redemptions_count, status, description, issued_time, expired_time, created_time, updated_time, _links].hash
    end
