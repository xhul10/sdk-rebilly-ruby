  ...
  class KhelocardCard
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::KhelocardCard` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::KhelocardCard`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      method_validator = EnumAttributeValidator.new('Object', ['Khelocard'])
    ...
    def method=(method)
      validator = EnumAttributeValidator.new('Object', ['Khelocard'])
    ...
    def hash
      [id, method, customer_id, fingerprint, number, last4, exp_year, exp_month, billing_address, status, created_time, updated_time, custom_fields, risk_metadata, _links, _embedded].hash
    end
