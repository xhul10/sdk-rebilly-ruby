...
module RebillySdk
  ...
  class NetbankingCredentials
    # Netbanking MID code.
    attr_accessor :midcode

    # Netbanking MID secret key.
    attr_accessor :midsecret
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::NetbankingCredentials` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::NetbankingCredentials`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'midcode')
        self.midcode = attributes[:'midcode']
      end

      if attributes.key?(:'midsecret')
        self.midsecret = attributes[:'midsecret']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @midcode.nil?
        invalid_properties.push('invalid value for "midcode", midcode cannot be nil.')
      end

      if @midsecret.nil?
        invalid_properties.push('invalid value for "midsecret", midsecret cannot be nil.')
      end
    ...
    def valid?
      return false if @midcode.nil?
      return false if @midsecret.nil?
      true
    end
