...
module RebillySdk
  ...
  class TWINTCredentials
    # TWINT store UUID.
    attr_accessor :store_uuid

    # Cash register ID.
    attr_accessor :cash_register_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::TWINTCredentials` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::TWINTCredentials`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'store_uuid')
        self.store_uuid = attributes[:'store_uuid']
      end

      if attributes.key?(:'cash_register_id')
        self.cash_register_id = attributes[:'cash_register_id']
      end
    ...
    def list_invalid_properties
      ...
      if @store_uuid.nil?
        invalid_properties.push('invalid value for "store_uuid", store_uuid cannot be nil.')
      end

      if @cash_register_id.nil?
        invalid_properties.push('invalid value for "cash_register_id", cash_register_id cannot be nil.')
      end
    ...
    def valid?
      return false if @store_uuid.nil?
      return false if @cash_register_id.nil?
    ...
    def hash
      [store_uuid, cash_register_id, public_key, private_key, key_passphrase].hash
    end
