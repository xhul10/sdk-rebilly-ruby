  ...
  class CommonOrder
    ...
    def self.openapi_types
      {
        ...
        :'website_id' => :'Object',
        :'currency' => :'Object',
        :'initial_invoice_id' => :'Object',
        :'recent_invoice_id' => :'Object',
        :'items' => :'Object',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::CommonOrder` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::CommonOrder`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      order_type_validator = EnumAttributeValidator.new('Object', ['subscription-order', 'one-time-order'])
      return false unless order_type_validator.valid?(@order_type)
      billing_status_validator = EnumAttributeValidator.new('Object', ['draft', 'unpaid', 'past-due', 'abandoned', 'paid', 'voided', 'refunded', 'disputed', 'partially-refunded', 'partially-paid'])
    ...
    def order_type=(order_type)
      validator = EnumAttributeValidator.new('Object', ['subscription-order', 'one-time-order'])
    ...
    def billing_status=(billing_status)
      validator = EnumAttributeValidator.new('Object', ['draft', 'unpaid', 'past-due', 'abandoned', 'paid', 'voided', 'refunded', 'disputed', 'partially-refunded', 'partially-paid'])
    ...
    def hash
      [id, order_type, billing_status, website_id, currency, initial_invoice_id, recent_invoice_id, items, delivery_address, billing_address, activation_time, void_time, coupon_ids, po_number, shipping, notes].hash
    end
