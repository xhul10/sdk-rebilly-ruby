  ...
  class PaysafeCredentials
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::PaysafeCredentials` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::PaysafeCredentials`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'store_pwd')
        self.store_pwd = attributes[:'store_pwd']
      end
    ...
    def list_invalid_properties
      ...
      if @store_pwd.nil?
        invalid_properties.push('invalid value for "store_pwd", store_pwd cannot be nil.')
      end

      if @account_num.nil?
        invalid_properties.push('invalid value for "account_num", account_num cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @store_pwd.nil?
      return false if @account_num.nil?
      true
    end
    ...
    def hash
      [store_id, store_pwd, account_num].hash
    end
