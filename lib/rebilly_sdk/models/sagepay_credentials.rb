  ...
  class SagepayCredentials
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::SagepayCredentials` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::SagepayCredentials`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'m_id')
        self.m_id = attributes[:'m_id']
      end

      if attributes.key?(:'m_key')
        self.m_key = attributes[:'m_key']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @m_id.nil?
        invalid_properties.push('invalid value for "m_id", m_id cannot be nil.')
      end

      if @m_key.nil?
        invalid_properties.push('invalid value for "m_key", m_key cannot be nil.')
      end
    ...
    def valid?
      return false if @m_id.nil?
      return false if @m_key.nil?
      true
    end
