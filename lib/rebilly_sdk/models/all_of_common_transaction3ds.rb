...
module RebillySdk
  class AllOfCommonTransaction3ds
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::AllOfCommonTransaction3ds` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::AllOfCommonTransaction3ds`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      version_validator = EnumAttributeValidator.new('', ['1.0.2', '2.1.0', '2.2.0'])
      return false unless version_validator.valid?(@version)
      enrolled_validator = EnumAttributeValidator.new('', ['yes', 'no', 'invalid card/timeout', 'unavailable'])
      return false unless enrolled_validator.valid?(@enrolled)
      authenticated_validator = EnumAttributeValidator.new('', ['yes', 'no', 'not applicable', 'attempted'])
      return false unless authenticated_validator.valid?(@authenticated)
      liability_validator = EnumAttributeValidator.new('', ['protected', 'not protected', 'protected (attempt)'])
      return false unless liability_validator.valid?(@liability)
      flow_validator = EnumAttributeValidator.new('', ['frictionless', 'challenge'])
    ...
    def version=(version)
      validator = EnumAttributeValidator.new('', ['1.0.2', '2.1.0', '2.2.0'])
    ...
    def enrolled=(enrolled)
      validator = EnumAttributeValidator.new('', ['yes', 'no', 'invalid card/timeout', 'unavailable'])
    ...
    def authenticated=(authenticated)
      validator = EnumAttributeValidator.new('', ['yes', 'no', 'not applicable', 'attempted'])
    ...
    def liability=(liability)
      validator = EnumAttributeValidator.new('', ['protected', 'not protected', 'protected (attempt)'])
    ...
    def flow=(flow)
      validator = EnumAttributeValidator.new('', ['frictionless', 'challenge'])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          flow == o.flow &&
          is_downgraded == o.is_downgraded && super(o)
    end
