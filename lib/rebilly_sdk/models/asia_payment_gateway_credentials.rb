...
module RebillySdk
  ...
  class AsiaPaymentGatewayCredentials
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::AsiaPaymentGatewayCredentials` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::AsiaPaymentGatewayCredentials`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'merchant_number')
        self.merchant_number = attributes[:'merchant_number']
      end
    ...
    def list_invalid_properties
      ...
      if @merchant_number.nil?
        invalid_properties.push('invalid value for "merchant_number", merchant_number cannot be nil.')
      end
    ...
    def valid?
      return false if @merchant_number.nil?
    ...
    def hash
      [merchant_number, secret_key].hash
    end
