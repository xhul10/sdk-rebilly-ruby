  ...
  class CCAvenueCredentials
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::CCAvenueCredentials` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::CCAvenueCredentials`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'working_key')
        self.working_key = attributes[:'working_key']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @access_code.nil?
        invalid_properties.push('invalid value for "access_code", access_code cannot be nil.')
      end

      if @working_key.nil?
        invalid_properties.push('invalid value for "working_key", working_key cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @working_key.nil?
      true
    end
    ...
    def hash
      [merchant_id, access_code, working_key].hash
    end
