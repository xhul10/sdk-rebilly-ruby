  ...
  class MtaPaySettings
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::MtaPaySettings` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::MtaPaySettings`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'goods')
        self.goods = attributes[:'goods']
      end

      if attributes.key?(:'mobile_pay')
        self.mobile_pay = attributes[:'mobile_pay']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @goods.nil?
        invalid_properties.push('invalid value for "goods", goods cannot be nil.')
      end

      if @mobile_pay.nil?
        invalid_properties.push('invalid value for "mobile_pay", mobile_pay cannot be nil.')
      end
    ...
    def valid?
      return false if @goods.nil?
      return false if @mobile_pay.nil?
      true
    end
