  ...
  class ThreeDSecureResult
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::ThreeDSecureResult` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::ThreeDSecureResult`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'is_downgraded')
        self.is_downgraded = attributes[:'is_downgraded']
      else
    ...
    def valid?
      version_validator = EnumAttributeValidator.new('Object', ['1.0.2', '2.1.0', '2.2.0'])
      return false unless version_validator.valid?(@version)
      enrolled_validator = EnumAttributeValidator.new('Object', ['yes', 'no', 'invalid card/timeout', 'unavailable'])
      return false unless enrolled_validator.valid?(@enrolled)
      authenticated_validator = EnumAttributeValidator.new('Object', ['yes', 'no', 'not applicable', 'attempted'])
      return false unless authenticated_validator.valid?(@authenticated)
      liability_validator = EnumAttributeValidator.new('Object', ['protected', 'not protected', 'protected (attempt)'])
      return false unless liability_validator.valid?(@liability)
      flow_validator = EnumAttributeValidator.new('Object', ['frictionless', 'challenge'])
    ...
    def version=(version)
      validator = EnumAttributeValidator.new('Object', ['1.0.2', '2.1.0', '2.2.0'])
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] enrolled Object to be assigned
    def enrolled=(enrolled)
      validator = EnumAttributeValidator.new('Object', ['yes', 'no', 'invalid card/timeout', 'unavailable'])
      unless validator.valid?(enrolled)
        fail ArgumentError, "invalid value for \"enrolled\", must be one of #{validator.allowable_values}."
      end
      @enrolled = enrolled
    end
    ...
    def authenticated=(authenticated)
      validator = EnumAttributeValidator.new('Object', ['yes', 'no', 'not applicable', 'attempted'])
      unless validator.valid?(authenticated)
        fail ArgumentError, "invalid value for \"authenticated\", must be one of #{validator.allowable_values}."
      end
      @authenticated = authenticated
    end
    ...
    def liability=(liability)
      validator = EnumAttributeValidator.new('Object', ['protected', 'not protected', 'protected (attempt)'])
      unless validator.valid?(liability)
        fail ArgumentError, "invalid value for \"liability\", must be one of #{validator.allowable_values}."
      end
      @liability = liability
    end
    ...
    def flow=(flow)
      validator = EnumAttributeValidator.new('Object', ['frictionless', 'challenge'])
    ...
    def hash
      [server, version, enrolled, authenticated, liability, flow, is_downgraded].hash
    end
