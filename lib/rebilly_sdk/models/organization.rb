  ...
  class Organization
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::Organization` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::Organization`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'tax_descriptor')
        self.tax_descriptor = attributes[:'tax_descriptor']
      end

      if attributes.key?(:'invoice_time_zone')
        self.invoice_time_zone = attributes[:'invoice_time_zone']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          emails == o.emails &&
          tax_descriptor == o.tax_descriptor &&
          invoice_time_zone == o.invoice_time_zone &&
    ...
    def hash
      [id, created_time, updated_time, name, address, address2, city, region, country, postal_code, phone_numbers, emails, tax_descriptor, invoice_time_zone, is_primary, questionnaire, settings, _links].hash
    end
