...
module RebillySdk
  # STP Mexico credentials object.
  class STPMexicoCredentials
    # STP Mexico account company name.
    attr_accessor :company_name

    # The name of the bank account holder receiving payments.
    attr_accessor :beneficiary_name

    attr_accessor :bank_id

    # The account number of the bank account receiving payments.
    attr_accessor :bank_account_number

    # STP Mexico private key.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::STPMexicoCredentials` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::STPMexicoCredentials`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [company_name, beneficiary_name, bank_id, bank_account_number, private_key, key_passphrase].hash
    end
