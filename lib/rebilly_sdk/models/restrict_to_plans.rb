...
module RebillySdk
  ...
  class RestrictToPlans
    ...
    # Minimum total quantity on which to apply the restriction and subsequent discount.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::RestrictToPlans` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::RestrictToPlans`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @plan_ids.nil?
        invalid_properties.push('invalid value for "plan_ids", plan_ids cannot be nil.')
      end
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('', ['discounts-per-redemption', 'minimum-order-amount', 'restrict-to-invoices', 'restrict-to-plans', 'restrict-to-subscriptions', 'restrict-to-products', 'paid-by-time'])
      return false unless type_validator.valid?(@type)
      return false if @plan_ids.nil?
      true
    end
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('', ['discounts-per-redemption', 'minimum-order-amount', 'restrict-to-invoices', 'restrict-to-plans', 'restrict-to-subscriptions', 'restrict-to-products', 'paid-by-time'])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          type == o.type &&
          plan_ids == o.plan_ids &&
          minimum_quantity == o.minimum_quantity && super(o)
    end
    ...
    def hash
      [type, plan_ids, minimum_quantity].hash
    end
