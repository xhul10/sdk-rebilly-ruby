...
module RebillySdk
  ...
  class AllOfSubscriptionMetadataRiskMetadata
    # The customer's IP.
    ...
    def self.openapi_types
      {
        ...
        :'accuracy_radius' => :'',
        :'distance' => :'',
        :'has_mismatched_billing_address_country' => :'',
        :'has_mismatched_bank_country' => :'',
        :'has_mismatched_time_zone' => :'',
        :'has_mismatched_holder_name' => :'',
        :'has_fake_name' => :'',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::AllOfSubscriptionMetadataRiskMetadata` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::AllOfSubscriptionMetadataRiskMetadata`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
