...
module RebillySdk
  ...
  class OrganizationSettingsKyc
    attr_accessor :identity_proof

    attr_accessor :address_proof
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::OrganizationSettingsKyc` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::OrganizationSettingsKyc`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'identity_proof')
        self.identity_proof = attributes[:'identity_proof']
      end

      if attributes.key?(:'address_proof')
        self.address_proof = attributes[:'address_proof']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          identity_proof == o.identity_proof &&
          address_proof == o.address_proof
    end
    ...
    def hash
      [identity_proof, address_proof].hash
    end
