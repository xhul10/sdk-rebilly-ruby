  ...
  class InvoiceTimeline
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::InvoiceTimeline` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::InvoiceTimeline`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'occurred_time')
        self.occurred_time = attributes[:'occurred_time']
      end
    ...
    def valid?
      type_validator = EnumAttributeValidator.new('Object', ['timeline-comment-created', 'invoice-created', 'invoice-issued', 'invoice-abandoned', 'invoice-voided', 'invoice-past-due', 'invoice-paid', 'invoice-partially-paid', 'invoice-disputed', 'invoice-refunded', 'invoice-partially-refunded', 'invoice-renewal-payment-declined', 'email-message-sent', 'coupon-applied', 'transaction-approved', 'transaction-abandoned', 'transaction-canceled', 'transaction-declined', 'transaction-initiated', 'transaction-refunded', 'transaction-voided'])
      return false unless type_validator.valid?(@type)
      triggered_by_validator = EnumAttributeValidator.new('Object', ['rebilly', 'app', 'direct-api'])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('Object', ['timeline-comment-created', 'invoice-created', 'invoice-issued', 'invoice-abandoned', 'invoice-voided', 'invoice-past-due', 'invoice-paid', 'invoice-partially-paid', 'invoice-disputed', 'invoice-refunded', 'invoice-partially-refunded', 'invoice-renewal-payment-declined', 'email-message-sent', 'coupon-applied', 'transaction-approved', 'transaction-abandoned', 'transaction-canceled', 'transaction-declined', 'transaction-initiated', 'transaction-refunded', 'transaction-voided'])
    ...
    def triggered_by=(triggered_by)
      validator = EnumAttributeValidator.new('Object', ['rebilly', 'app', 'direct-api'])
    ...
    def hash
      [id, type, triggered_by, message, extra_data, occurred_time, _links].hash
    end
