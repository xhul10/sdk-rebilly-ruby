  ...
  class CustomerJWT
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::CustomerJWT` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::CustomerJWT`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'invalidate')
        self.invalidate = attributes[:'invalidate']
      else
      ...
      if attributes.key?(:'custom_claims')
        if (value = attributes[:'custom_claims']).is_a?(Hash)
    ...
    def valid?
      type_validator = EnumAttributeValidator.new('Object', ['customer'])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('Object', ['customer'])
    ...
    def hash
      [id, type, token, invalidate, one_time_password, customer_id, acl, custom_claims, created_time, updated_time, expired_time, _links].hash
    end
