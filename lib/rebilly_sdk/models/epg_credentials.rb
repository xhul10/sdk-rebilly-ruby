  ...
  class EPGCredentials
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::EPGCredentials` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::EPGCredentials`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @merchant_key.nil?
        invalid_properties.push('invalid value for "merchant_key", merchant_key cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @merchant_key.nil?
      true
    end
    ...
    def hash
      [merchant_id, product_id, merchant_password, merchant_key].hash
    end
