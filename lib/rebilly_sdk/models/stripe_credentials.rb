...
module RebillySdk
  # Stripe Connect credentials object. The credentials object may be provided in order to control the Stripe Connect account under which the gateway account is connected.
  class StripeCredentials
    # The Client ID under which to connect the Stripe account.
    attr_accessor :stripe_client_id

    # The secret key of the Stripe Connect account under which to connect the Stripe account.
    attr_accessor :stripe_secret
    ...
    def self.openapi_types
      {
        :'stripe_client_id' => :'Object',
        :'stripe_secret' => :'Object'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::StripeCredentials` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::StripeCredentials`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'stripe_client_id')
        self.stripe_client_id = attributes[:'stripe_client_id']
      end

      if attributes.key?(:'stripe_secret')
        self.stripe_secret = attributes[:'stripe_secret']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          stripe_client_id == o.stripe_client_id &&
          stripe_secret == o.stripe_secret
    end
    ...
    def hash
      [stripe_client_id, stripe_secret].hash
    end
