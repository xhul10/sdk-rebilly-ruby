...
module RebillySdk
  ...
  class EMerchantPaySettings
    # Use Genesis platform on eMerchantPay.
    attr_accessor :use_genesis_platform

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'use_genesis_platform' => :'useGenesisPlatform'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::EMerchantPaySettings` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::EMerchantPaySettings`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'use_genesis_platform')
        self.use_genesis_platform = attributes[:'use_genesis_platform']
      else
        self.use_genesis_platform = false
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          use_genesis_platform == o.use_genesis_platform
    end
