  ...
  class BamboraCredentials
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::BamboraCredentials` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::BamboraCredentials`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'api_passcode')
        self.api_passcode = attributes[:'api_passcode']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @api_passcode.nil?
        invalid_properties.push('invalid value for "api_passcode", api_passcode cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @api_passcode.nil?
      true
    end
