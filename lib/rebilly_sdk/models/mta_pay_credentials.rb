  ...
  class MtaPayCredentials
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::MtaPayCredentials` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::MtaPayCredentials`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'md5key')
        self.md5key = attributes[:'md5key']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @md5key.nil?
        invalid_properties.push('invalid value for "md5key", md5key cannot be nil.')
      end
    ...
    def hash
      [account_id, party_id, md5key].hash
    end
