  ...
  class PaymeroSettings
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::PaymeroSettings` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::PaymeroSettings`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'main_currency')
        self.main_currency = attributes[:'main_currency']
      end

      if attributes.key?(:'amount_exceeded')
        self.amount_exceeded = attributes[:'amount_exceeded']
      else
        self.amount_exceeded = false
      end
    end
    ...
    def valid?
      main_currency_validator = EnumAttributeValidator.new('Object', ['TRX', 'ETH'])
      return false unless main_currency_validator.valid?(@main_currency)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] main_currency Object to be assigned
    def main_currency=(main_currency)
      validator = EnumAttributeValidator.new('Object', ['TRX', 'ETH'])
      unless validator.valid?(main_currency)
        fail ArgumentError, "invalid value for \"main_currency\", must be one of #{validator.allowable_values}."
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          main_currency == o.main_currency &&
          amount_exceeded == o.amount_exceeded
    end
    ...
    def hash
      [target_currency, main_currency, amount_exceeded].hash
    end
