  ...
  class InvoiceTax
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::InvoiceTax` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::InvoiceTax`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'calculator')
        self.calculator = attributes[:'calculator']
      end
    ...
    def list_invalid_properties
      ...
      if @calculator.nil?
        invalid_properties.push('invalid value for "calculator", calculator cannot be nil.')
      end
    ...
    def valid?
      return false if @calculator.nil?
      calculator_validator = EnumAttributeValidator.new('Object', ['manual', 'rebilly-taxjar'])
      return false unless calculator_validator.valid?(@calculator)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] calculator Object to be assigned
    def calculator=(calculator)
      validator = EnumAttributeValidator.new('Object', ['manual', 'rebilly-taxjar'])
      unless validator.valid?(calculator)
        fail ArgumentError, "invalid value for \"calculator\", must be one of #{validator.allowable_values}."
      end
      @calculator = calculator
    end
