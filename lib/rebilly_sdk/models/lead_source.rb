  ...
  class LeadSource
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::LeadSource` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::LeadSource`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'sub_affiliate')
        self.sub_affiliate = attributes[:'sub_affiliate']
      end

      if attributes.key?(:'sales_agent')
        self.sales_agent = attributes[:'sales_agent']
      end
    ...
    def hash
      [medium, source, campaign, term, content, affiliate, sub_affiliate, sales_agent, click_id, path, referrer, created_time, _links, original].hash
    end
