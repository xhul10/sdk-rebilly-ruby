...
module RebillySdk
  class PayClubSettings
    # Use 3DS channel.
    attr_accessor :use3_ds_channel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::PayClubSettings` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::PayClubSettings`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'use3_ds_channel')
        self.use3_ds_channel = attributes[:'use3_ds_channel']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          use3_ds_channel == o.use3_ds_channel
    end
