  ...
  class CommonPaymentCard
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::CommonPaymentCard` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::CommonPaymentCard`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      method_validator = EnumAttributeValidator.new('Object', ['payment-card'])
      return false unless method_validator.valid?(@method)
      status_validator = EnumAttributeValidator.new('Object', ['active', 'inactive', 'expired', 'deactivated', 'verification-needed'])
    ...
    def method=(method)
      validator = EnumAttributeValidator.new('Object', ['payment-card'])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('Object', ['active', 'inactive', 'expired', 'deactivated', 'verification-needed'])
    ...
    def hash
      [id, method, customer_id, status, fingerprint, bin, last4, pan, exp_year, exp_month, cvv, brand, bank_country, bank_name, billing_address, created_time, updated_time, custom_fields, risk_metadata].hash
    end
