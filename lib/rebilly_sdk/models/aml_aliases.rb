  ...
  class AMLAliases
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::AMLAliases` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::AMLAliases`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      authenticity_validator = EnumAttributeValidator.new('Object', ['strong', 'weak', 'unknown'])
      return false unless authenticity_validator.valid?(@authenticity)
      true
    end
    ...
    def authenticity=(authenticity)
      validator = EnumAttributeValidator.new('Object', ['strong', 'weak', 'unknown'])
      unless validator.valid?(authenticity)
        fail ArgumentError, "invalid value for \"authenticity\", must be one of #{validator.allowable_values}."
      end
      @authenticity = authenticity
    end
    ...
    def hash
      [first_name, last_name, authenticity].hash
    end
