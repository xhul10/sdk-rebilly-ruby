  ...
  class DayOfWeek
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::DayOfWeek` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::DayOfWeek`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      method_validator = EnumAttributeValidator.new('', ['immediately', 'date-interval', 'day-of-month', 'day-of-week'])
      return false unless method_validator.valid?(@method)
      return false if @day.nil?
      week_validator = EnumAttributeValidator.new('', ['next', 'first-in-month', 'last-in-month'])
      return false unless week_validator.valid?(@week)
      true
    end
    ...
    def method=(method)
      validator = EnumAttributeValidator.new('', ['immediately', 'date-interval', 'day-of-month', 'day-of-week'])
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] week Object to be assigned
    def week=(week)
      validator = EnumAttributeValidator.new('', ['next', 'first-in-month', 'last-in-month'])
      unless validator.valid?(week)
        fail ArgumentError, "invalid value for \"week\", must be one of #{validator.allowable_values}."
      end
      @week = week
    end
