...
module RebillySdk
  ...
  class AstroPayCardSettings
    # Use One Touch AstroPay API.
    attr_accessor :one_touch_api
    ...
    def self.openapi_types
      {
        :'one_touch_api' => :'Object',
        :'merchant_name' => :'Object',
        :'merchant_logo_url' => :'Object'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::AstroPayCardSettings` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::AstroPayCardSettings`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'one_touch_api')
        self.one_touch_api = attributes[:'one_touch_api']
      else
        self.one_touch_api = false
      end
    ...
    def hash
      [one_touch_api, merchant_name, merchant_logo_url].hash
    end
