  ...
  class IssueTimeShiftInstruction
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::IssueTimeShiftInstruction` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::IssueTimeShiftInstruction`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @chronology.nil?
        invalid_properties.push('invalid value for "chronology", chronology cannot be nil.')
      end
    ...
    def valid?
      return false if @chronology.nil?
      chronology_validator = EnumAttributeValidator.new('Object', ['before'])
      return false unless chronology_validator.valid?(@chronology)
    ...
    # @param [Object] chronology Object to be assigned
    def chronology=(chronology)
      validator = EnumAttributeValidator.new('Object', ['before'])
      unless validator.valid?(chronology)
        fail ArgumentError, "invalid value for \"chronology\", must be one of #{validator.allowable_values}."
      end
      @chronology = chronology
    end
