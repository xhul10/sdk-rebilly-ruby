...
module RebillySdk
  class PercentageFormula
    ...
    # A mode how the rounding should work: floor, ceiling, or regular rounding: - \"up\" (ceiling) example: $0.011 becomes $0.02, $0.019 becomes $0.02; - \"down\" (floor) example: $0.011 becomes $0.01, $0.019 becomes $0.01; - \"regular\" example: $0.011 becomes $0.01, $0.019 becomes $0.02, $0.015 becomes 0.02 too; Defaults to \"regular\". 
    attr_accessor :round
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RebillySdk::PercentageFormula` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RebillySdk::PercentageFormula`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'bips')
        self.bips = attributes[:'bips']
      end
    ...
    def list_invalid_properties
      ...
      if @bips.nil?
        invalid_properties.push('invalid value for "bips", bips cannot be nil.')
      end
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('Object', ['percentage'])
      return false unless type_validator.valid?(@type)
      return false if @currency.nil?
      return false if @bips.nil?
      round_validator = EnumAttributeValidator.new('Object', ['regular', 'up', 'down'])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('Object', ['percentage'])
    ...
    def round=(round)
      validator = EnumAttributeValidator.new('Object', ['regular', 'up', 'down'])
    ...
    def hash
      [type, currency, bips, round, min_amount].hash
    end
